    ...
    signTicket = callback => {
      ...
      signHelper
        .sign({
          ...
          key: path.join(
            __dirname,
            '../' + config.get('certs.path') + companyId + '.privada'
          ),
          cert: path.join(
            __dirname,
            '../' + config.get('certs.path') + companyId + '.crt'
          ),
          password: companyId
        })
        .catch(err => {
        ...
        client.loginCms(args, (err, result) => {
          if (err) {
            ...
            parseString(err.body, (err2, parsedErr) => {
              if (err2) {
                return callback(
                  'parseString: Error parsing string. Error: ' + err2
                );
              }
              ...
              let afipError = {
                ...
                message:
                  parsedErr['soapenv:Envelope']['soapenv:Body'][0][
                    'soapenv:Fault'
                  ][0].faultstring[0]
              };
